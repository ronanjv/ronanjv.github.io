...
## Part i
Now that we have all the things we need, let's try to print the recipe. Below, I have written code to print from the objects. Remove `#| eval: false` from the YAML header and test. Let me know if you're stuck!

**Image**
![yum](website/images/KimchiGrilledCheese.jpg)
...
**Answer**
While there is no strict yes or no to whether it is legal to scrape data from public websites, there is a lot of specific circumstances. For instance, the company may prohibit their data being used or if it is behind a paywall it is illegal. It depends more on how the data is being scraped. Here, however, it is easier to use a fake website in which we know there are no resctrictions by the website to scraping the public data.
...
**Answer**
Yes I did use conditionals in Part j to classify the numbers in column x1 as low, mid, or high. Othere than that conditional statements were not needed.

## Loops
Did you use loops in your code? If yes, how. If not, are there places you could have used them but did something else?

**Answer**
Yes I used loops in this lab. I used a for loop in part h to fill in the x12 column and part i when applying the calculate.score(...) function to each entry in the matrix. I also used a loop for the menu.qmd file where I iterated over each random recipe to gather the information needed for the picture, ingredients, and instructions. While I used loops already  to scrape multiple recipes, they could have been used more consistently in the printing stage rather than repeating four times.

## Functions
Did you use functions in your code? If yes, how. If not, are there places you could have used them but did something else?

**Answer**
I used functions throughout both the lab write-up and Menu.qmd. In the lab write-up, I wrote custom functions such as calculate.score() and evaluate.row() to encapsulate repeated logic and apply it consistently across the data frame. In Menu.qmd, I relied on built-in functions like read_html(), html_element(), and html_elements() to scrape and process recipe data. I could have made a function to print the data in menu.qmd rather than repeating the steps for each piece of the recipe four times.
