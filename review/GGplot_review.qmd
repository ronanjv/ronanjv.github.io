...
ggplot(data, aes(x = "", y = score)) +
  geom_violin(fill = "darkorchid", alpha = .5) +
...
Once you are comfortable with the core geoms, ggplot2 has some powerful tools 
that take your plots to the next level.

**Facet Wrap and Patchwork**  
Facet Wrap can be used to put graphs next to and on top of eachother. Another option is patchwork. p1 / p2 will put graph 1 on top of graph 2. p1 | p2 or p1 + p2 will put them side by side.
```{r}
#| label: FacetWrap
#| fig-cap: "Facet Wrapped Graphs"
#| fig-width: 5
#| fig-height: 3
#| fig-align: "center"
...
**Geom_ribbon()**  
Geom_ribbon() allows us to highlight under a portion of the graph. For example, below we have normal distribution looking data with the upper 20% highlighted.
```{r}
#| label: geomribbonGraph
#| fig-cap: "Upper 20%"
#| fig-width: 5
#| fig-height: 3
#| fig-align: "center"
...
ribbon_data = data.frame(x = x, y = y)

ggplot(ribbon_data, aes(x = x, y = y)) +
  geom_line() +
  geom_ribbon(data = subset(ribbon_data, x >= cutoff),
              aes(ymin = 0, ymax = y), fill = "darkorchid", alpha = 0.5) +
  labs(x = "X", y = "Density") +
  theme_bw()
```

**Transparency**  
Using alpha values, we can make the color of our graph look more transparent.

```{r}
#| label: alphaValuesGraphs
#| fig-cap: "Transparency"
#| fig-width: 8
#| fig-height: 3
#| fig-align: "center"
...
p1 = ggplot(ribbon_data, aes(x = x, y = y)) +
  geom_line() +
  geom_ribbon(data = subset(ribbon_data, x >= cutoff),
              aes(ymin = 0, ymax = y), fill = "darkorchid", alpha = 0.9) +
  labs(x = "X", y = "Density") +
  theme_bw()

p2 = ggplot(ribbon_data, aes(x = x, y = y)) +
  geom_line() +
  geom_ribbon(data = subset(ribbon_data, x >= cutoff),
              aes(ymin = 0, ymax = y), fill = "darkorchid", alpha = 0.6) +
  labs(x = "X", y = "Density") +
  theme_bw()

p3 = ggplot(ribbon_data, aes(x = x, y = y)) +
  geom_line() +
  geom_ribbon(data = subset(ribbon_data, x >= cutoff),
              aes(ymin = 0, ymax = y), fill = "darkorchid", alpha = 0.3) +
  labs(x = "X", y = "Density") +
  theme_bw()

p4 = ggplot(ribbon_data, aes(x = x, y = y)) +
  geom_line() +
  geom_ribbon(data = subset(ribbon_data, x >= cutoff),
              aes(ymin = 0, ymax = y), fill = "darkorchid", alpha = 0.1) +
  labs(x = "X", y = "Density") +
  theme_bw()
